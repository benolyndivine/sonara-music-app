...
export default function ArtistDetailsView({ artist, songs, artists = [], currentTrack, isPlaying, isShuffle, onToggleShuffle, onTogglePlay, onSelectTrack, onSelectArtist, onBack }) {
  ...
  const similarArtists = artists.filter(a => a.id !== artist.id).slice(0, 5);
  ...
  return (
    <div className="mobile-content" style={{ width: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box', padding: 0, paddingBottom: '160px', position: 'relative' }}>
      ...
      {/* 🌟 Similar Artists / Fans Also Liked Row */}
      {similarArtists.length > 0 && (
        <div style={{ padding: '20px 20px 0 20px', marginTop: '16px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <h3 style={{ fontSize: '1.1rem', color: '#fff', marginBottom: '14px', fontWeight: '700' }}>Fans Also Liked</h3>
          <div className="horizontal-scroll" style={{ display: 'flex', gap: '16px', overflowX: 'auto', paddingBottom: '10px' }}>
            {similarArtists.map((simArtist) => (
              <div
                key={simArtist.id}
                onClick={() => onSelectArtist && onSelectArtist(simArtist)}
                style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '100px', maxWidth: '100px' }}
              >
                {simArtist.image ? (
                  <img src={simArtist.image} alt={simArtist.name} style={{ width: '90px', height: '90px', borderRadius: '50%', objectFit: 'cover' }} />
                ) : (
                  <div style={{ width: '90px', height: '90px', borderRadius: '50%', backgroundColor: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#000', fontWeight: '700' }}>
                    {(simArtist.name || '?').charAt(0).toUpperCase()}
                  </div>
                )}
                <p style={{ fontSize: '0.85rem', color: '#fff', margin: '8px 0 0 0', textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100px' }}>
                  {simArtist.name}
                </p>
              </div>
            ))}
