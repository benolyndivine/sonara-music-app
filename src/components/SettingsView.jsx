...
export default function SettingsView({
  ...
  songs = [], // 🆕 Accept current songs library array to audit files
  onBack
}) {
  const [cachedCount, setCachedCount] = useState(0);

  // 🆕 Streaming / download quality — persisted directly to localStorage via
  // utils/audioQuality.js (same pattern as saved artists / offline tracks),
  // so no prop plumbing through App.jsx is needed and the choice survives reload.
  ...
  // Recalculate local download metrics on mount
  useEffect(() => {
    let count = 0;
  ...
  return (
    <div className="mobile-content set-hide-scrollbar" style={{ paddingBottom: '40px', overflowY: 'auto' }}>
      ...
      {/* 🆕 SECTION: LOCAL STORAGE AUDIT MANAGEMENT */}
      ...
      <div style={{ marginTop: '48px', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '16px', textAlign: 'center' }}>
        ...
        <p style={{ margin: 0, fontSize: '0.7rem', color: 'rgba(255,255,255,0.2)', fontWeight: '700', letterSpacing: '0.5px' }}>VERSION 1.4.3 (1546)</p>
      </div>
    </div>
  );
}
