...
import { formatTimestampTag, parseSyncedLyrics } from '../utils/lyricsSync';
...
export default function ManageLyricsView({ songs = [], lyrics = [], onBack, onLyricsSaved }) {
  ...
  // 🎧 Timestamp-tagging preview player — lets an admin play the actual
  // track and stamp the current playback time onto whichever lyrics line
  // their cursor is on, so synced lyrics can be authored by ear.
  ...
  const handleStampLine = () => {
    const tag = formatTimestampTag(previewTime);
    const textarea = textareaRef.current;
    if (!textarea) return;

    const cursor = textarea.selectionStart ?? lyricsText.length;
    const lineStart = lyricsText.lastIndexOf('\n', cursor - 1) + 1;
    const lineEnd = (() => {
      const idx = lyricsText.indexOf('\n', lineStart);
      return idx === -1 ? lyricsText.length : idx;
    })();

    const line = lyricsText.slice(lineStart, lineEnd);
    const lineWithoutExistingTag = line.replace(/^\s*\[\d{1,2}:\d{2}(?:[.:]\d{1,3})?\]\s*/, '');
    const newLine = `${tag} ${lineWithoutExistingTag}`;
    const nextText = lyricsText.slice(0, lineStart) + newLine + lyricsText.slice(lineEnd);
    setLyricsText(nextText);

    // Move cursor to the end of the line just stamped, and jump the
    // preview forward a touch so tapping repeatedly walks down the lyrics.
  ...
  const previewParsed = parseSyncedLyrics(lyricsText);
        ...
        <div style={cardStyle}>
          ...
          <form onSubmit={handleSaveLyrics} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <label style={{ ...labelStyle, marginBottom: 0 }}>Lyrics Lines</label>
              <span style={{ fontSize: '0.7rem', fontWeight: '700', color: previewParsed.isSynced ? 'var(--accent)' : 'var(--text-muted)' }}>
                {previewParsed.isSynced ? '🎤 Synced' : 'Plain text'}
              </span>
            </div>
            <p style={{ margin: '-8px 0 0 0', fontSize: '0.72rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>
              Optional: start a line with a timestamp like <code>[00:12]</code> to make it karaoke-synced in the player.
              Play the track below and tap "Stamp Line" to insert the current time onto whichever line your cursor is on.
            </p>

            {previewUrl && (
              <div style={{
                display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px',
                borderRadius: 10, backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)'
              }}>
                <audio
                  ref={previewAudioRef}
                  src={previewUrl}
                ...
                <span style={{ fontSize: '0.78rem', color: '#fff', fontVariantNumeric: 'tabular-nums', minWidth: 40 }}>
                  {formatTimestampTag(previewTime).slice(1, -1)}
                </span>
                <button
                  type="button"
                  onClick={handleStampLine}
                  style={{
                    marginLeft: 'auto', padding: '6px 12px', borderRadius: 8, border: 'none',
                    backgroundColor: 'rgba(255,255,255,0.1)', color: '#fff', fontSize: '0.75rem',
                    fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6
                  }}
                >
                  <i className="fa-solid fa-stamp"></i> Stamp Line
                </button>
              </div>
            )}
