...
export default function ProfileDrawerView({
  ...
  const handleCheckUpdate = () => {
    if (updateInfo && updateInfo.version && updateInfo.version !== currentVersion) {
      const confirmUpdate = window.confirm(`Update v${updateInfo.version} is available!\n\nNotes: ${updateInfo.notes || 'Performance improvements'}\n\nDo you want to download it now?`);
      if (confirmUpdate && updateInfo.url) {
        window.open(updateInfo.url, '_blank');
      }
    } else {
      alert(`Checking system servers... Sonara is completely up to date! (v${currentVersion})`);
    }
  };
  ...
  return (
    ...
    >
      ...
      {isHeadAdmin && (
        <div style={{ marginBottom: '18px', backgroundColor: 'rgba(29, 185, 84, 0.04)', padding: '16px 14px', borderRadius: '16px', border: '1px solid rgba(29, 185, 84, 0.18)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <button onClick={onOpenManageGenres} className="pdv-action-row" style={adminRowBtnStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={iconBoxStyle}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', backgroundColor: 'rgba(255, 255, 255, 0.02)', borderRadius: '16px', border: '1px solid rgba(255, 255, 255, 0.06)', overflow: 'hidden', marginBottom: '24px' }}>
        ...
        <button className="pdv-list-row" onClick={onNavigateSettings} style={listRowStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={iconBoxStyle}>
        ...
        <button className="pdv-list-row" onClick={handleCheckUpdate} style={listRowStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={iconBoxStyle}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              {updateInfo && updateInfo.version && updateInfo.version !== currentVersion && (
                <span style={{ backgroundColor: 'var(--accent)', color: '#000', fontSize: '0.65rem', fontWeight: '800', padding: '2px 6px', borderRadius: '10px' }}>NEW</span>
              )}
            </div>
          </div>
          <i className="fa-solid fa-chevron-right" style={chevronStyle}></i>
...
const profileDrawerStyles = `
  ...
  .pdv-hide-scrollbar::-webkit-scrollbar { display: none; }
  .pdv-hide-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
  ...
  .pdv-list-row:hover { background-color: rgba(255, 255, 255, 0.04) !important; }
